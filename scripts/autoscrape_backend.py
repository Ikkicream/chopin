...
def notify_telegram(message: str) -> None:
    # Le .env declare TELEGRAM_BOT_TOKEN — c est le nom qu emploie `alertes.py`, le seul
    # emetteur qui ait jamais fonctionne. Ici on lisait TELEGRAM_TOKEN, qui n existe pas :
    # `token` restait vide, la fonction sortait sans rien dire, et TOUTES les alertes
    # passant par elle (nettoyage nocturne, veilleur de verrou) etaient perdues en
    # silence depuis toujours. Constate le 2026-09-21 en verifiant que le veilleur
    # pourrait bel et bien prevenir Camille.
    token = _env("TELEGRAM_TOKEN") or _env("TELEGRAM_BOT_TOKEN")
    chat = _env("TELEGRAM_CHAT_ID") or _env("TELEGRAM_CHAT")
