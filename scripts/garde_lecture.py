...
-- Les fiches DISTINCTES vues, qui est la seule mesure honnête d'une exfiltration.
-- `lecture_contacts` additionne des lignes servies : rouvrir trois fois la même liste de
-- 227 contacts y compte 681 « fiches lues ». C'est ce qui a valu à Romeo une alerte
-- « ASPIRATION DE LA BASE » le 2026-09-09 pour trois ouvertures de page en 93 secondes —
-- 1 185 lignes, zéro fiche nouvelle après la première. Un commercial qui recharge sa
-- liste d'appels entre deux coups de fil travaille ; il n'aspire rien.
-- L'ancienne table reste : c'est le journal (qui, quand, combien de lignes servies).
-- Celle-ci porte le quota.
CREATE TABLE IF NOT EXISTS lecture_fiches (
    utilisateur text NOT NULL,
    contact_ref text NOT NULL,
    vu_le       timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_fiches_user ON lecture_fiches (utilisateur, vu_le DESC);

-- Marque les lectures dont on connaît les fiches : celles-là sont comptées dans
-- `lecture_fiches`, il ne faut surtout pas les compter une seconde fois au volume.
ALTER TABLE lecture_contacts ADD COLUMN IF NOT EXISTS identifiee boolean NOT NULL DEFAULT false;
"""
    ...
    try:
        with c:
            with c.cursor() as cur:
                cur.execute(SCHEMA)
                # Le quota ne regarde qu'une heure glissante ; garder des mois de lignes
                # ne sert qu'à faire grossir la base. Une fois par process suffit.
                cur.execute("DELETE FROM lecture_fiches WHERE vu_le < now() - interval '2 days'")
    finally:
        p._rendre(c)
    _table_prete = True
...
def lues_derniere_heure(utilisateur: str) -> int:
    """Fiches DISTINCTES vues dans l'heure glissante.

    On compte des contacts, pas des lignes servies. Une route qui ne sait pas dire QUELLES
    fiches elle a servies retombe sur son volume brut (`lecture_contacts`) : mieux vaut une
    mesure grossière que pas de garde-fou du tout. Les deux s'additionnent, chacune sur son
    périmètre — une même route ne peut pas alimenter les deux.
    """
    if not utilisateur:
        return 0
    try:
        _assurer_table()
        distinctes = _pool()._q(
            "SELECT count(DISTINCT contact_ref) FROM lecture_fiches "
            "WHERE utilisateur = %s AND vu_le >= now() - interval '1 hour'",
        ...
        brutes = _pool()._q(
            ...
            "AND NOT identifiee",
            (utilisateur,))[0][0] or 0
        return int(distinctes) + int(brutes)
...
def journaliser(utilisateur: str, role: str | None, site: str, route: str,
                lignes: int, identifiants: list | None = None) -> None:
    ...
    try:
        ...
        try:
            with c:
                with c.cursor() as cur:
                    refs = [str(x) for x in (identifiants or []) if x]
                    cur.execute(
                        "INSERT INTO lecture_contacts "
                        "(utilisateur, role, site_code, route, lignes, identifiee) "
                        "VALUES (%s, %s, %s, %s, %s, %s)",
                        (utilisateur, role, site, route, int(lignes), bool(refs)))
                    # Quand la route sait QUI elle a servi, on retient les fiches : c'est
                    # elles qui portent le quota, et relire les mêmes ne coûte plus rien.
                    if refs:
                        cur.executemany(
                            "INSERT INTO lecture_fiches (utilisateur, contact_ref) VALUES (%s, %s)",
                            [(utilisateur, r) for r in refs])
        finally:
            p._rendre(c)
