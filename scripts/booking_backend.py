...
def _conn(retries: int = 8, delay: float = 0.4):
    """Ouvre god_mode par `duck_ouverture`, comme tout le reste du process.

    Ce module tourne DANS l'API, aux côtés d'une quinzaine d'autres qui ouvrent la même
    base. DuckDB refuse une seconde connexion dont la configuration diffère de celles déjà
    ouvertes — et `duckdb.connect()` en dur, ici, ne parlait pas la même langue que
    `duck_ouverture.ouvrir()` qu'emploie l'API. D'où, en boucle :

        Can't open a connection to same database file with a different configuration

    La boucle de ré-essai ci-dessous ne rattrapait rien : elle ne réagissait qu'à
    « Conflicting lock », un VERROU, alors que ce message-ci est un conflit de
    CONFIGURATION. Le premier essai échouait, et les sept autres ne servaient à rien.
    Conséquence visible : la pastille « réservations non lues » restait vide sans jamais
    dire pourquoi, l'appel étant avalé côté interface par un `.catch(() => null)`.
    """
    import sys as _sys
    _sys.path.insert(0, str(Path(__file__).resolve().parent))
    from duck_ouverture import ouvrir
    return ouvrir(GOD_DB, tentatives=retries, pause_s=delay)
...
def set_status(site: str, bid: str, status: str) -> dict:
    ...
    c = _conn()
    try:
        # On relit AVANT d'écrire : l'appelant a besoin de savoir de quel client il s'agit
        # pour porter la trace dans sa fiche. Sans ça, « marquer répondu » archivait le
        # rendez-vous et n'en laissait aucune trace côté contact — le commercial suivant
        # ouvrait la fiche sans jamais apprendre qu'une demande avait été traitée.
        ligne = c.execute(
            "SELECT email, name, reason_label, slot_start FROM bookings "
            "WHERE id=? AND site_code=?", [bid, site]).fetchone()
    ...
    return {"ok": True, "id": bid, "status": status,
            "email": ligne[0], "name": ligne[1],
            "reason_label": ligne[2], "slot_start": ligne[3]}
