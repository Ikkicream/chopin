...
# Cooldowns (cf. specs/contacts-model.md §3.2 + §3.5)
COOLDOWN_GLOBAL_DAYS = 30
COOLDOWN_SAME_SITE_DAYS = 7
...
def list_contacts_for_site(site_code: str, state: list[str] | None = None,
    ...
    if source:
        ...
        q += f" AND csh.source IN ({placeholders})"
        params.extend(source)
    if search_email:
        q += " AND lower(c.email) LIKE ?"
        params.append(f"%{search_email.lower()}%")
    # sectors_in : on filtre par texte JSON LIKE pour simplicité (DuckDB)
    if sectors_in:
...
def stats_for_site(site_code: str) -> dict:
    ...
    try:
        total = c.execute(
            "SELECT COUNT(*) FROM contact_site_history WHERE site_code = ?", [site_code]
        ).fetchone()[0]
        by_state = dict(c.execute(
            "SELECT state, COUNT(*) FROM contact_site_history WHERE site_code = ? GROUP BY state",
            [site_code]
        ).fetchall())
        by_source = dict(c.execute(
            "SELECT source, COUNT(*) FROM contact_site_history WHERE site_code = ? GROUP BY source",
            [site_code]
        ).fetchall())
    finally:
        c.close()
