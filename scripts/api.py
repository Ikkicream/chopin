...
import re          # NB 2026-08-26 : n'était importé que localement dans 6 fonctions,
                   # d'où un NameError sur les 4 routes avatars (_nom_avatar).
import time
...
# Mass Mailing (superadmin) — module indépendant, dans `mass_mailing/` à la racine.
# Monté sous garde : si le module ne s'importe pas, Cheffer démarre quand même et
# seules les routes /api/mass-mailing manquent (Camille, 2026-09-25 : « tout doit
# être dans un dossier indépendant »).
try:
    from mass_mailing.api.routes import router as _mass_mailing_router
    from mass_mailing.api.routes import public_router as _mass_mailing_public
    app.include_router(_mass_mailing_router)
    app.include_router(_mass_mailing_public)   # lien de désinscription, sans session
except Exception as _e:  # noqa: BLE001
    print(f"[api] Mass Mailing non monté : {type(_e).__name__}: {_e}", flush=True)
...
def _credits_fournisseurs() -> dict:
    """Ce qu'il reste chez les deux fournisseurs de collecte.

    Les deux ne se racontent pas pareil, et la barre ne doit pas faire semblant du
    contraire :

    - Serper expose un solde en direct, et on journalise chaque appel — on peut donc
      dire le solde ET le rythme auquel il descend. Un solde sans rythme ne dit rien :
      4 994 crédits, c'est six mois si Basile porte la collecte, deux jours s'il tombe.
    - Basile n'expose RIEN. Son API n'a aucune route de quota (cf. docs/basile-api.md
      §3) : on apprend qu'il est à sec quand il répond 402, pas avant. On affiche donc
      un état, pas un chiffre inventé — et la date de retour, qui elle est connue :
      le quota mensuel du plan `api` repart le 1er du mois à 00 h 00 UTC.
    """
    import time as _t
    from datetime import datetime, timezone

    out = {"serper": {"solde": None, "ok": False, "conso_jour": None,
                      "conso_24h": None, "jours_restants": None},
           "basile": {"bloque": False, "http": None, "depuis": None,
                      "quota_mensuel": 250000, "reset_le": None}}

    # ── Serper : solde live + rythme de consommation ──────────────────────────
    try:
        _scripts_dans_le_chemin()
        import god_mode_agents as _gma
        bal = _gma.serper_balance() or {}
        out["serper"]["solde"] = bal.get("balance")
        out["serper"]["ok"] = bool(bal.get("ok"))
    except Exception:  # noqa: BLE001 — un solde absent vaut mieux qu'une barre cassée
        pass
    try:
        c = _duck(BASE_DIR / "data" / "god_mode.duckdb")
        try:
            # Moyenne sur 7 jours plutôt que la journée en cours : à 9 h du matin, la
            # conso du jour dit toujours « il reste une éternité ».
            conso = c.execute(
                "SELECT coalesce(sum(credits), 0) / 7.0 FROM god_mode_serper_calls "
                "WHERE created_at > now() - INTERVAL 7 DAY").fetchone()[0] or 0
            # Et les dernières 24 h, qui disent une chose différente : Serper brûle-t-il
            # MAINTENANT ? Les deux ne se confondent pas. Le 04/09, la moyenne 7 jours
            # valait 1 170/j — entièrement due aux quatre jours où Basile était à sec —
            # alors que Serper n'avait plus rien consommé depuis le 1er. Afficher la seule
            # moyenne aurait annoncé « 4 jours restants » sur un robinet fermé.
            recent = c.execute(
                "SELECT coalesce(sum(credits), 0) FROM god_mode_serper_calls "
                "WHERE created_at > now() - INTERVAL 1 DAY").fetchone()[0] or 0
        finally:
            c.close()
        conso = round(float(conso))
        out["serper"]["conso_jour"] = conso
        out["serper"]["conso_24h"] = round(float(recent))
        solde = out["serper"]["solde"]
        if solde is not None and conso > 0:
            # Autonomie au rythme des 7 derniers jours : à lire comme « si Basile
            # retombe », pas comme une échéance. C'est la question qui compte, puisque
            # Serper ne sert qu'à ça.
            out["serper"]["jours_restants"] = round(solde / conso, 1)
    except Exception:  # noqa: BLE001
        pass

    # ── Basile : un état, et la date de retour du quota ───────────────────────
    # Même règle de fraîcheur que `alertes.py` : personne n'efface `fournisseur_bloque`,
    # il vit dans le statut du dernier run. Passé six heures sans être réécrit, c'est un
    # fossile — sinon la barre annoncerait « Basile épuisé » jusqu'à la fin des temps,
    # y compris après le 1er du mois quand le quota est revenu.
    try:
        _scripts_dans_le_chemin()
        import autoscrape_backend as _asb
        for site in ("lcr", "mkd"):
            fb = ((_asb.read_status(site) or {}).get("fournisseur_bloque") or {})
            if not fb or fb.get("source") != "basile":
                continue
            quand = fb.get("quand")
            try:
                age_h = (datetime.now(timezone.utc)
                         - datetime.fromisoformat(str(quand))).total_seconds() / 3600
            except Exception:  # noqa: BLE001
                continue
            if age_h <= 6:
                out["basile"].update(bloque=True, http=fb.get("http"), depuis=quand)
                break
    except Exception:  # noqa: BLE001
        pass

    maintenant = datetime.now(timezone.utc)
    an, mois = (maintenant.year + 1, 1) if maintenant.month == 12 else (maintenant.year, maintenant.month + 1)
    out["basile"]["reset_le"] = datetime(an, mois, 1, tzinfo=timezone.utc).isoformat()
    return out
...
async def api_superadmin_bar(request: Request):
    ...
    # Cache 5 min : le solde Serper est un appel réseau, la barre interroge toutes les
    # 15 s, et un solde de crédits ne bouge pas à la seconde.
    if _t.time() - _CREDITS_CACHE["ts"] > 300 or _CREDITS_CACHE["data"] is None:
        try:
            _CREDITS_CACHE.update(ts=_t.time(), data=_credits_fournisseurs())
        except Exception:  # noqa: BLE001
            pass

    return {"ok": True, "ip": ip, "users_online": users_online,
            "users_online_list": users_online_list, "campaigns_routing": routing,
            "credits": _CREDITS_CACHE["data"]}
    ...
    if t:
        try:
            ...
            if email and "@" in email:
                ...
                canal = info.get("channel") or "maildoso"
                try:
                    _cpb.record_engagement(site, email, "open", canal)
                    # Pixel maison : c'est notre propre lien, pas un relais Sweego — pas
                    # de drapeau proxy à transmettre, le défaut convient.
                except Exception as _err:
                    # `contacts.duckdb` n'accepte qu'UN écrivain : tout batch qui la tient
                    # (nettoyage nocturne, enrichissement, import) fait échouer cette
                    # écriture. Tant que l'échec était simplement imprimé, l'ouverture
                    # disparaissait — le 2026-09-21 une journée entière est partie ainsi,
                    # pixel renvoyant 200 et personne pour s'en apercevoir. On la dépose
                    # dans la file de secours, `verrou_watchdog.py` la rejouera.
                    import engagement_spool as _spool
                    _spool.deposer(site, email, "open", canal)
                    print(f"[track][open] différée (base occupée) : {_err}")
        except Exception as _e:
...
async def api_sweego_webhook(request: Request):
    ...
    # Mass Mailing (campagnes « mm-… ») est un autre métier : ses statistiques viennent
    # des logs Sweego et ses désinscriptions de son propre lien. Un clic sur un mail de
    # masse ne doit pas faire passer un contact Cheffer en lead (2026-09-25).
    if str(campaign_id).startswith("mm-"):
        return {"ok": True, "ignore": "mass-mailing"}
...
def api_onoff_messagerie(site: str, non_lus: bool = False, limite: int = 100,
                         lus: bool = False):
    """`non_lus` = la liste de travail, `lus` = les archives, aucun des deux = tout."""
...
def api_a_rappeler(site: str, request: Request, vue: str = "mes"):
    ...
    try:
        ...
        fiches = (res.get("contacts") or res.get("a_rappeler") or []) if isinstance(res, dict) else []
        n = len(fiches)
        # On dit QUELLES fiches ont été servies, pas seulement combien. Le garde compte des
        # contacts distincts : recharger sa liste d'appels entre deux coups de fil ne doit
        # plus ressembler à une aspiration de la base.
        refs = [f.get("contact_id") or f.get("email") for f in fiches
                if isinstance(f, dict) and (f.get("contact_id") or f.get("email"))]
        gl.journaliser(user, role, site, "a-rappeler", n, identifiants=refs)
        return res
    except Exception as e:  # noqa: BLE001
        from fastapi.responses import JSONResponse
...
@app.post("/api/sites/{site}/a-rappeler/faire-rappeler")
async def api_a_rappeler_faire_rappeler(site: str, request: Request):
    """« À rappeler par le responsable » — transmettre SANS annoncer une signature.

    Il manquait la marche du milieu. Un commercial n'avait que deux gestes : garder le
    contact, ou appuyer sur « Le client dit oui » — qui envoie des confettis et annonce une
    affaire conclue. Entre les deux, le cas le plus fréquent n'avait pas de bouton : le
    prospect mérite un rappel par quelqu'un d'autre, et on ne sait pas encore s'il signera.
    Faute de geste juste, on prenait le geste faux, et le tableau des ventes s'en trouvait
    faussé.

    Ici : pas de célébration, pas d'opportunité créée, aucun statut de vente touché. Le
    contact change de mains et la RAISON voyage avec lui — c'est elle qui fait la valeur du
    transfert. 160 caractères : de quoi dire « rappelle-le, il hésite sur le prix », pas de
    quoi écrire un rapport que personne ne lira.
    """
    sys.path.insert(0, str(BASE_DIR / "scripts"))
    import followup_backend as fb
    from fastapi.responses import JSONResponse
    ...
    d = await request.json()
    email = (d.get("email") or "").strip()
    raison = " ".join((d.get("raison") or "").split())
    if not email:
        return JSONResponse(status_code=400, content={"ok": False, "error": "contact manquant"})
    if not raison:
        return JSONResponse(status_code=400,
                            content={"ok": False, "error": "la raison est obligatoire — "
                                                           "c'est elle qui sert au responsable"})
    if len(raison) > 160:
        return JSONResponse(status_code=400,
                            content={"ok": False, "error": f"raison trop longue ({len(raison)}/160)"})

    responsable = fb.commercial_par_defaut(site)
    if not responsable:
        return JSONResponse(status_code=409,
                            content={"ok": False, "error": "aucun responsable configuré pour ce site"})

    r = fb.assigner(site, email, responsable, role, user)
    if not r.get("ok"):
        return JSONResponse(status_code=403, content=r)

    # La trace vaut le transfert : six mois plus tard, on doit pouvoir dire POURQUOI ce
    # contact a changé de mains, et qui l'a demandé.
    try:
        fb.journaliser(site, email, user or "inconnu", "rappel_responsable",
                       f"À rappeler par {responsable} — {raison}")
    except Exception:  # noqa: BLE001 — le transfert a eu lieu, le journal ne doit pas l'annuler
        pass
    return {"ok": True, "responsable": responsable, "raison": raison}
...
@app.get("/api/sites/{site}/templates/tableau")
def api_templates_tableau(site: str):
    """La galerie des cold emails : contenu, état, et ce qu'ils ont produit."""
    _scripts_dans_le_chemin()
    import email_templates_backend as etb
    try:
        return etb.tableau(site)
    except Exception as e:  # noqa: BLE001
        from fastapi.responses import JSONResponse
        return JSONResponse(status_code=503, content={"error": f"tableau indisponible: {e}"[:200]})


@app.get("/api/sites/{site}/templates/{sector}/{kind}/rendu")
def api_template_rendu(site: str, sector: str, kind: str):
    """Le message TEL QUE LE DESTINATAIRE LE RECEVRA.

    L'aperçu montrait le gabarit brut : `{{si prenom}}Bonjour {{prenom}},{{sinon}}…`, sans
    retour à la ligne ni pied de page. On ne pouvait donc pas juger ce qui part réellement
    — ce qui est pourtant la seule chose qu'on veuille voir avant d'envoyer.

    On rend ici les DEUX cas, parce que 71 % des contacts n'ont pas de prénom : celui qui
    en a un, et celui qui n'en a pas. Le pied de page légal est inclus.
    """
    _scripts_dans_le_chemin()
    import email_templates_backend as etb, html_templates_backend as htb
    import qualite_message as qm, maildoso_backend as md, expediteur as ex
    t = etb._get_one(site, sector, kind)
    if not t:
        return _introuvable("message")
    emballe = htb.resolve_campaign_message(site, f"cold:{sector}:{kind}")
    brut = (emballe or {}).get("html") or t.get("body_html") or ""

    try:
        boites = ex.boites(site)
        boite = next((b for b in boites if b.get("usage") == "mozart"), boites[0] if boites else {})
    except Exception:  # noqa: BLE001
        boite = {"email": "immo@leclient-roi.com", "sender_name": "Julie Durand"}

    exemples = [
        ("avec prénom", {"email": "m.dupont@exemple.fr", "prenom": "Marc", "nom": "Dupont",
                         "societe": "Agence Dupont", "city": "Nantes"}),
        ("sans prénom (71 % des contacts)",
         {"email": "contact@exemple.fr", "prenom": "", "nom": "",
          "societe": "Agence Dupont", "city": "Nantes"}),
    ]
    rendus = []
    for libelle, ct in exemples:
        try:
            h = qm.conditionnel(qm.spintax(brut, ct["email"]), ct)
            h = md._apply_tokens(h, ct, boite)
            o = md._apply_tokens(qm.conditionnel(qm.spintax(t["subject"] or "", ct["email"]), ct),
                                 ct, boite)
        except Exception as e:  # noqa: BLE001
            h, o = brut, (t.get("subject") or "")
            print(f"[templates] rendu impossible ({type(e).__name__}: {e})", flush=True)
        rendus.append({"cas": libelle, "objet": o, "html": h,
                       "expediteur": f"{boite.get('sender_name','')} <{boite.get('email','')}>"})
    return {"sector": sector, "kind": kind, "rendus": rendus,
            "note": ("Rendu réel, pied de page légal compris. Le spintax est tiré une fois : "
                     "chaque destinataire recevra une variante.")}
...
def api_message_rendu(site: str, modele: str = "", brut: bool = False):
    ...
    « cold email et newsletters sont la même chose »). Une route qui n'accepte que
    `secteur/kind` ne sait rien afficher d'une newsletter, dont l'identité est
    `struct:<fichier>` ou `ver:<uuid>`.

    `resolve_campaign_message` sait déjà résoudre les trois formes — c'est elle que le
    dispatch utilise pour savoir quoi envoyer. On s'appuie sur la même, sinon l'aperçu
    montrerait autre chose que ce qui part.
    """
    _scripts_dans_le_chemin()
    import html_templates_backend as htb, qualite_message as qm
    ...
    return {"modele": modele, "nom": (resolu or {}).get("name") or modele,
            "rendus": [{"cas": "aperçu", "objet": (resolu or {}).get("name") or "", "html": h,
                        "expediteur": f"{boite.get('sender_name','')} <{boite.get('email','')}>"}],
            "note": "Aperçu du message tel qu'il est enregistré, variables développées."}


@app.post("/api/sites/{site}/templates/{sector}/{kind}/favori")
async def api_template_favori(site: str, sector: str, kind: str, request: Request):
    """Marque ou retire un favori. Body : {favori: bool}."""
    try:
        d = await request.json()
    except Exception:  # noqa: BLE001
        d = {}
    _scripts_dans_le_chemin()
    import email_templates_backend as etb
    return etb.set_favori(site, sector, kind, bool(d.get("favori", True)))
...
async def booking_set_status(site: str, bid: str, request: Request):
    """« Marquer répondu » : le rendez-vous part aux archives, ET la fiche du client le sait.

    L'archivage marchait déjà ; la trace côté contact, non — elle n'avait jamais été
    écrite. Un commercial qui rouvrait la fiche ne pouvait pas savoir qu'une demande de
    rendez-vous avait été reçue, ni qu'on y avait répondu. Rien n'est déclaré gagné ici :
    on consigne un fait, on ne conclut aucune affaire.
    """
    sys.path.insert(0, str(BASE_DIR / "scripts"))
    import booking_backend as bb
    role, user = _qui(request)
    body = await request.json()
    statut = body.get("status", "answered")
    r = bb.set_status(site, bid, statut)

    if r.get("ok") and r.get("email"):
        libelle = {"answered": "répondu", "cancelled": "annulé",
                   "confirmed": "remis à traiter"}.get(statut, statut)
        detail = f"Demande de rendez-vous « {r.get('reason_label') or 'sans motif'} »"
        if r.get("slot_start"):
            detail += f" du {r['slot_start']}"
        detail += f" — {libelle} par {user or 'inconnu'}"
        try:
            import followup_backend as fb
            fb.journaliser(site, r["email"], user or "inconnu", "booking", detail)
        except Exception:  # noqa: BLE001 — le rendez-vous est archivé, le journal ne l'annule pas
            pass
    return r
...
async def api_pool_contact_create(site: str, request: Request):
    ...
    # Un contact sans email — celui qu'on n'a eu qu'au téléphone. La base est bâtie sur
    # l'email comme clé : plutôt que de la rendre facultative partout, on fabrique une
    # adresse de remplacement, `client+N@cheffer.com`, unique et jamais envoyable.
    #
    # ⚠️ Cette adresse est DÉLIVRABLE : le « + » est du sous-adressage, tout ce qu'on lui
    # enverrait atterrirait dans la boîte client@cheffer.com. Elle n'existe que comme clé.
    # C'est `email_validator.est_adresse_de_remplacement` qui garantit qu'aucune campagne
    # ne la vise — le garde est là-bas, pas ici, parce que c'est là que passent TOUS les
    # chemins d'envoi.
    if body.get("sans_email") and not (body.get("email") or "").strip():
        if not (body.get("tel") or "").strip():
            return {"ok": False, "error": "un contact sans email doit au moins avoir un téléphone"}
        try:
            import email_validator as _ev
            body["email"] = _ev.prochaine_adresse_de_remplacement()
            body["sans_email_reel"] = True
        except Exception as e:  # noqa: BLE001
            return {"ok": False, "error": f"adresse de remplacement indisponible: {e}"[:150]}
