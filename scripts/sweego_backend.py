...
# ── Envoi ───────────────────────────────────────────────────────────────────────
class EnvoiGroupeInterdit(RuntimeError):
    """Un appel allait envoyer UN mail visible par plusieurs destinataires."""


def _verrou_un_a_un(route: str, body: dict) -> None:
    """Dernier rempart avant tout appel email : `/send` n'a droit qu'à UNE adresse,
    sans copie ni copie cachée. Lève plutôt que d'envoyer : un envoi refusé se
    rattrape, des adresses exposées à des inconnus, jamais."""
    if route != "/send" or body.get("channel") == "sms" or body.get("provider") == "sms":
        return
    n = len(body.get("recipients") or []) + len(body.get("cc") or []) + len(body.get("bcc") or [])
    if n > 1:
        raise EnvoiGroupeInterdit(
            f"/send avec {n} destinataires = mail de groupe, interdit (1 destinataire = 1 mail)")
...
def send_campaign(campaign_id: str, subject: str, html_str: str,
    ...
    try:
        ...
        pass
    # ⚖️ 1 destinataire = 1 mail, TOUJOURS (Camille, 2026-09-25 : un mail de groupe
    # exposant les adresses est interdit par la loi). Sur Sweego, `/send` avec N
    # adresses envoie UN mail de groupe où chacun voit tous les autres dans « À »
    # (doc : .claude/skills/sweego/SKILL.md § 1). À partir de 2 destinataires on
    # passe donc par `/send/bulk/email`, qui envoie un mail séparé à chacun.
    route = "/send/bulk/email" if len(recipients) >= 2 else "/send"
    body = {
        "provider": "email",
    ...
    try:
        _verrou_un_a_un(route, body)
        r = requests.post(SWEEGO_URL + route, headers=_headers(), json=body, timeout=45)
        if r.status_code == 200:
            d = r.json()
            return {"ok": True, "transaction_id": d.get("transaction_id"),
                    "swg_uids": d.get("swg_uids"), "route": route,
...
def record_campaign(site, name, campaign_id, subject, sector, message_id,
    ...
    # PostgreSQL D'ABORD (fin du Lot 1) : Sweego ne journalise pas par destinataire, cette
    # ligne est donc la SEULE trace qu'un envoi de masse a eu lieu et vers combien de
    # personnes. La perdre sur un verrou DuckDB, c'est perdre l'envoi tout entier des
    # statistiques — même motif que le journal d'envoi, qui a été déplacé pour ça.
    try:
        import journal_pg
        journal_pg.enregistrer_envoi_masse(rid, site, name, campaign_id, subject, sector,
                                           message_id, count, transaction_id, by)
    except Exception as e:  # noqa: BLE001
        print(f"[sweego] envoi de masse non journalisé dans PostgreSQL : "
...
def list_campaigns(site) -> list[dict]:
    try:
        import journal_pg
        return journal_pg.lister_envois_masse(site)
    except Exception as e:  # noqa: BLE001
        print(f"[sweego] liste des envois de masse: PostgreSQL indisponible "
