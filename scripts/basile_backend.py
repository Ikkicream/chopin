...
def _req(path: str, body=None, method="POST", raw=False, with_headers=False,
    ...
    try:
        ...
        if e.code in (402, 403, 429):
            ...
            #
            # Le 429 n'arrive ICI qu'après cinq reprises espacées par le Retry-After : ce
            # n'est plus une limite passagère, c'est un refus qui dure. Le docstring
            # l'annonçait depuis toujours ; le code, lui, le laissait tomber dans l'erreur
            # générique — donc `run_sector_for_dept` renvoyait "error" et non "blocked", la
            # collecte croyait Basile vivant et le rappelait pour CHAQUE ville restante,
            # cinq reprises et cinq pauses à chaque fois, sans qu'aucune alerte ne parte.
            BASILE_BLOCKED_STATUS = e.code
            corps = e.read().decode(errors="replace")
            # Basile ne dit JAMAIS son reliquat sur une reponse normale — seulement dans
            # le corps d un 402 : {"quota":{"monthlyLimit":250000,"remaining":0}}. C est
            # la seule occasion de recaler le compteur local sur la verite du fournisseur,
            # alors on la saisit au vol plutot que de la laisser filer dans un message
            # d erreur tronque.
            try:
                q = (json.loads(corps) or {}).get("quota") or {}
                if "remaining" in q:
                    import basile_quota as _bq
                    _bq.reconcilier(int(q["remaining"]))
            except Exception:  # noqa: BLE001
                pass
            raise RuntimeError(f"BASILE_BLOCKED ({e.code}) sur {path}: {corps[:200]}")
...
def run_segment(site: str, kind: str, filters: dict, *, sector: str | None = None,
    ...
    if dry_run and samples:
        out["samples"] = samples
    out["status"] = "done"
    # Ce que cette passe a coute et rapporte. Sans cette ligne, `collected`/`valid` sont
    # calcules puis jetes a chaque run : c est ce qui rendait le quota du 21/09
    # impossible a anticiper et le rendement par secteur impossible a comparer.
    try:
        import basile_quota as _bq
        _bq.enregistrer(out, filters)
    except Exception:  # noqa: BLE001 — la comptabilite ne fait jamais echouer la collecte
        pass
    return out
...
# Toutes les N recherches d'email, on redemande le solde à Emelia. Le compteur local ne
# suffit pas : d'autres chemins consomment aussi (recherche de téléphone, vérifications),
# et un crédit dépensé ailleurs pendant la passe ne se verrait pas.
CONTROLE_SOLDE_TOUS_LES = 25


def _solde_emelia() -> int | None:
    """Le solde RÉEL chez Emelia, ou None s'il est illisible.

    Le compteur local (`emelia_credits.remaining()`) rend `None` tant que personne n'a
    saisi de solde de référence — donc un garde-fou fondé sur lui ne se déclencherait
    jamais. On interroge la source.
    """
    try:
        import emelia_credits
        live = emelia_credits.fetch_live_balance()
        if live.get("ok") and live.get("remaining") is not None:
...
def run_dirigeant_segment(site: str, companies_filter: dict, *, sector: str | None = None,
                          ...
                          budget_credits: int | None = None, plancher_credits: int = 0,
    ...
    #
    # ── Le garde-fou de crédits ──────────────────────────────────────────────
    # La boucle appelait Emelia pour CHAQUE dirigeant, sans jamais regarder le solde.
    # À court de crédits, elle continuait d'appeler : autant d'échecs, et aucune trace
    # de la raison. « Quand il n'y a plus de crédit, il n'y a plus de crédit » (Camille,
    # 2026-08-23) — alors on s'arrête, proprement, et on le dit.
    #
    # Le solde est relu chez Emelia : le compteur local rend `None` tant qu'aucun solde
    # de référence n'a été saisi, donc un garde-fou fondé sur lui ne se déclencherait
    # jamais. Et s'il est ILLISIBLE, on refuse de partir : dépenser sans pouvoir compter
    # est le seul cas où mieux vaut ne rien faire.
    solde = _solde_emelia()
    out["solde_emelia_avant"] = solde
    if use_emelia:
        if solde is None:
            out["status"] = "solde_emelia_illisible"
            out["note"] = ("impossible de connaître le solde Emelia — passe annulée "
                           "plutôt que de dépenser à l'aveugle")
            return out
        disponible = max(0, solde - max(0, plancher_credits))
        plafond = min(disponible, budget_credits) if budget_credits else disponible
        out["plafond_credits"] = plafond
        if plafond <= 0:
            out["status"] = "credits_epuises"
            out["note"] = f"solde {solde}, plancher {plancher_credits} — rien à dépenser"
            return out
    else:
        plafond = 0
    ...
    for c, dg in pairs:
        if use_emelia:
            if out.get("credits_factures", 0) >= plafond:
                out["status"] = "credits_epuises"
                out["note"] = (f"plafond de {plafond} crédit(s) atteint — "
                               f"{len(pairs) - out['emelia_calls']} dirigeant(s) non traité(s)")
                break
            # Deuxième borne, sur les RECHERCHES : une passe où Emelia ne trouve jamais
            # rien ne coûterait rien mais tournerait indéfiniment. Large exprès — elle ne
            # doit se déclencher que sur un rendement anormal, pas sur un rendement moyen.
            if out["emelia_calls"] >= plafond * 20:
                out["status"] = "rendement_trop_faible"
                out["note"] = (f"{out['emelia_calls']} recherches pour "
                               f"{out.get('credits_factures', 0)} email(s) trouvé(s) — "
                               f"passe interrompue, quelque chose ne va pas")
                break
            # Contrôle périodique du solde réel : d'autres chemins consomment aussi.
            if out["emelia_calls"] and out["emelia_calls"] % CONTROLE_SOLDE_TOUS_LES == 0:
                vu = _solde_emelia()
                if vu is not None and vu <= plancher_credits:
                    out["status"] = "credits_epuises"
                    out["note"] = (f"solde Emelia retombé à {vu} — arrêt, "
                                   f"{len(pairs) - out['emelia_calls']} dirigeant(s) "
                                   f"non traité(s)")
                    break
        try:
            ...
            if use_emelia:
                ...
                # Emelia ne facture QUE lorsqu'elle trouve. Compter les recherches revient
                # donc à surestimer massivement la dépense : la passe du 2026-08-23 s'est
                # arrêtée après 100 recherches en n'ayant coûté que 11 crédits. Le
                # garde-fou était sûr — il ne dépasse jamais — mais il s'arrêtait sept
                # fois trop tôt. On compte ce qui est facturé.
                if email:
                    out["credits_factures"] = out.get("credits_factures", 0) + 1
            if not email:
                out["rejected"] += 1
                continue
            prospect = {
    ...
    # « done » ne doit pas écraser un motif d'arrêt déjà posé. La boucle sort sur
    # `credits_epuises` en le disant, et cette ligne le remplaçait par « terminé » — la
    # passe s'arrêtait à 5 dirigeants sur 40 et le rapport annonçait un succès. Le
    # compteur `emelia_calls` restait juste, mais plus personne ne savait POURQUOI la
    # passe était courte, ce qui est le pire des deux mondes.
    out.setdefault("status", "done")
    if out["status"] not in ("credits_epuises", "solde_emelia_illisible",
                            "rendement_trop_faible"):
        out["status"] = "done"
    return out
