...
def sync_blacklist(email: str, reason: str = "") -> bool:
    """Un contact blacklisté n'a plus sa place dans PostgreSQL : on le retire, on ne le
    marque pas. Le garder avec un drapeau, c'est accepter d'accumuler du non-contactable —
    précisément ce que le modèle en entonnoir refuse. Son journal reste."""
    return retirer_par_email(email)
