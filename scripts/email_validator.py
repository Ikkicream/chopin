...
# ── Adresses de remplacement : les contacts qu'on n'a qu'au téléphone ─────────────
#
# La base est bâtie sur l'email comme clé. Plutôt que de la rendre facultative dans chaque
# écran et chaque requête, un contact sans adresse reçoit une clé fabriquée :
# `client+1@cheffer.com`, `client+2@cheffer.com`, etc.
#
# ⚠️ DANGER, et c'est pour ça que le garde est ICI : ces adresses sont DÉLIVRABLES. Le
# « + » est du sous-adressage — tout ce qu'on leur enverrait arriverait pour de bon dans
# la boîte client@cheffer.com. Une campagne qui en viserait cent enverrait cent messages
# à nous-mêmes, et les compteurs d'ouverture mentiraient dans la foulée.
#
# `validate_and_score` est le point de passage unique de tous les chemins d'envoi
# (acquisition, basile, god_mode). Le rejet posé ici vaut donc pour tous, y compris ceux
# qui n'existent pas encore.
DOMAINE_REMPLACEMENT = "cheffer.com"
PREFIXE_REMPLACEMENT = "client+"
_MOTIF_REMPLACEMENT = re.compile(
    r"^client\+(\d+)@" + re.escape(DOMAINE_REMPLACEMENT) + r"$", re.IGNORECASE)


def est_adresse_de_remplacement(email: str) -> bool:
    """Cette adresse est-elle une clé fabriquée pour un contact sans email ?"""
    return bool(_MOTIF_REMPLACEMENT.match(normalize_email(email or "")))


def prochaine_adresse_de_remplacement() -> str:
    """La prochaine clé libre, `client+N@cheffer.com`.

    On lit le plus grand N déjà posé et on prend le suivant. Deux créations simultanées
    pourraient viser le même numéro : l'unicité de l'email dans le pool fait alors échouer
    la seconde, et on retente. Mieux vaut une erreur franche qu'un doublon silencieux.
    """
    import sys as _sys
    from pathlib import Path as _P
    _sys.path.insert(0, str(_P(__file__).resolve().parent))
    from duck_ouverture import ouvrir
    base = _P(__file__).resolve().parent.parent / "data" / "contacts.duckdb"
    n = 0
    try:
        c = ouvrir(base)
        try:
            rows = c.execute(
                "SELECT email FROM contacts WHERE lower(email) LIKE ?",
                [PREFIXE_REMPLACEMENT.lower() + "%@" + DOMAINE_REMPLACEMENT]).fetchall()
        finally:
            c.close()
        for (adresse,) in rows:
            m = _MOTIF_REMPLACEMENT.match(normalize_email(adresse or ""))
            if m:
                n = max(n, int(m.group(1)))
    except Exception:  # noqa: BLE001 — une base illisible ne doit pas rendre la clé aléatoire
        raise
    return f"{PREFIXE_REMPLACEMENT}{n + 1}@{DOMAINE_REMPLACEMENT}"
...
def validate_and_score(email_raw: str, prospect: dict) -> dict:
    ...
    # Étage 1 — Clé fabriquée. Avant la syntaxe, et non après, pour deux raisons : le motif
    # doit apparaître tel quel dans le journal (« syntax_invalid » ferait chercher au
    # mauvais endroit), et le rejet ne doit pas dépendre du hasard. Aujourd'hui EMAIL_REGEX
    # refuse le « + » — mais le sous-adressage est valide au sens de la RFC, et quelqu'un
    # finira par « corriger » cette régularité. Le jour où ça arrivera, ce garde-ci tiendra
    # toujours, et aucune campagne ne partira vers client+N@cheffer.com.
    if est_adresse_de_remplacement(email):
        return {"email": email, "valid": False, "score": 0,
                "decision": "drop", "reasons": ["adresse_de_remplacement"]}
