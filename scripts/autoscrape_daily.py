...
def seed_targets(site: str = SITE) -> dict:
    ...
    # On compare des ENSEMBLES, pas des totaux. Le compte global mélange les secteurs
    # actuels et ceux qui ont quitté la file (`interdit` : leurs lignes restent en base
    # pour le dédoublonnage). Le 2026-08-29, ajouter `concession-auto` et `opticien`
    # portait la file à 23 secteurs — exactement le nombre de secteurs déjà présents,
    # `avocat` et `comptable` occupant la place des deux nouveaux. Le total tombait juste,
    # le semis passait son tour, et les deux secteurs restaient INVISIBLES : jamais une
    # cible, jamais une erreur.
    c = _conn(read_only=True)
    try:
        presents = {r[0] for r in c.execute(
            "SELECT DISTINCT sector FROM autoscrape_targets WHERE site_code=?",
            [site]).fetchall()}
        have = c.execute("SELECT COUNT(*) FROM autoscrape_targets WHERE site_code=?",
                         [site]).fetchone()[0]
    finally:
        c.close()
    if not set(secteurs) - presents and have >= len(secteurs) * len(depts):
