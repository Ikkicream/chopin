...
SECTOR_QUERIES = {
    ...
    # Distinct de `garagiste` : on cherche des VENDEURS de véhicules, pas des réparateurs.
    # Les deux univers ne se recoupent presque pas côté Google Places, et « garage » seul
    # ramenait de la réparation — d'où un secteur qui semblait vide.
    "concession-auto": ["concessionnaire automobile {city}", "mandataire automobile {city}",
                        "concession auto {city}", "vente voiture occasion {city}",
                        "garage vente voiture {city}", "négociant automobile {city}",
                        "voiture neuve {city}"],
    "opticien":    ["opticien {city}", "magasin optique {city}", "opticien lunetier {city}",
                    "lunetterie {city}", "optique lunettes {city}"],
