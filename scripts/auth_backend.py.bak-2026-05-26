...
def update_user(user_id, data):
    conn = get_db()
    fields, values = [], []
    for k in ["role", "nom", "prenom", "email", "phone", "sites"]:
        if k in data:
            fields.append(f"{k} = ?")
...
def login(username, password, totp_code: str = "", ip: str = ""):
    ...
    conn = get_db()
    row = conn.execute(
        "SELECT id, password_hash, role, nom, prenom, totp_secret, totp_enabled, sites "
    ...
    user_id, stored_hash, role, nom, prenom, totp_secret, totp_enabled, sites = row
