...
# Le cron lance ce script à 6 h 30 UTC, et le créneau d'entretien lui est réservé jusqu'à
# 7 h 20 (`autoscrape_daily.ENTRETIEN_RESERVE_UTC`). La patience s'arrête donc à 7 h 15 :
# au-delà on empiéterait sur la reprise de la collecte, et trois tâches attendent derrière
# dans la même ligne de cron (datagouv_enrich, pg_sync_enrichment, pg_reconcile).
PATIENCE_S = 2700


def _patienter(ouvrir, patience_s: int = PATIENCE_S):
    """Ouvre une base DuckDB en attendant qu'un scrape lui rende la main.

    Le 2026-08-28 à 6 h 30, une passe lancée à 6 h 01 courait toujours : le rattrapage est
    mort sur le verrou et 108 contacts déjà collectés sont restés hors du pool jusqu'au
    soir — la journée s'est affichée à 207 contacts au lieu de 315. Le créneau d'entretien
    empêche de DÉMARRER une passe, il n'interrompt pas celle qui court : le conflit est
    structurel, pas accidentel.

    On n'attend QUE sur un verrou. Une base absente, corrompue ou refusée par les droits
    échouera pareil dans quarante-cinq minutes : réessayer à l'identique ne la réparera
    pas, et ferait passer une panne franche pour une lenteur — en retardant au passage les
    trois tâches de la chaîne. Même règle que `contacts_pool_backend._connect_with_retry`.
    """
    import time
    fin = time.monotonic() + patience_s
    while True:
        try:
            return ouvrir()
        except Exception as e:  # noqa: BLE001
            msg = str(e).lower()
            if not ("lock" in msg or "conflicting" in msg):
                raise
...
def _god(read_only: bool = True):
    import duckdb
    return _patienter(lambda: duckdb.connect(str(GOD_DB), read_only=read_only))
...
def analyser(depuis: str | None = None) -> dict:
    ...
    c = _patienter(lambda: pool._conn(read_only=True))
    try:
