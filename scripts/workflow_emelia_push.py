...
# ── Campagnes AUTO : création + push batch vers campagne existante ─────────────
def ensure_campaign_for_auto(site: str, sector: str) -> str | None:
    """Crée (ou récupère) la campagne Emelia {site}-{sector} — steps (templates verrouillés)
    + settings 8h-18h lun-ven + start + webhook — et renvoie son id. Réutilise
    get_or_create_campaign (idempotent : 1 campagne Emelia par secteur)."""
    api_key = _get_key(site)
    if not api_key:
        return None
    camp = get_or_create_campaign(api_key, site, sector)
    return _camp_id(camp)


def push_batch_to_campaign(site: str, campaign_id: str, sector: str, n: int,
                           sender_email: str | None = None) -> dict:
    """Pousse jusqu'à n contacts VALIDES du pool (secteur) vers une campagne Emelia
    DÉJÀ existante. Réutilise pick_for_campaign + POST contact + mark_pushed_to_emelia.

    Anti-double-push : pick_for_campaign exclut déjà cooldown / state != cold_email /
    déjà-poussés / blacklist / data.gouv exclus ; on ne marque le pool QUE si HTTP 200/201
    (et Emelia dédup côté serveur). Renvoie {pushed, picked, errors[]}."""
    if n <= 0:
        return {"pushed": 0, "picked": 0, "errors": []}
    api_key = _get_key(site)
    if not api_key:
        return {"pushed": 0, "picked": 0, "errors": ["no_emelia_key"]}
    import contacts_pool_backend as pool
    contacts = pool.pick_for_campaign(site, sector, limit=int(n))
    pushed = 0
    errors: list[str] = []
    for ct in contacts:
        ...
        contact = {
            ...
            "field3":    ct.get("dept_code") or "",
            "field4":    ct.get("website") or "",
        }
        try:
            r = requests.post(f"{EMELIA_URL}/emails/campaign/contacts",
            ...
            continue
        if r.status_code not in (200, 201):
            errors.append(f"{email}: http_{r.status_code}")
            continue
        emelia_cid = ""
        try:
            data = r.json()
            emelia_cid = data.get("_id") or data.get("id") or data.get("contactId") or ""
        except Exception:
            pass
        try:
            pool.mark_pushed_to_emelia(ct["id"], site, campaign_id, emelia_cid or "")
        except Exception as e:
            errors.append(f"mark {email}: {e}")
        pushed += 1
    return {"pushed": pushed, "picked": len(contacts), "errors": errors}
