...
def _total_pool(site: str) -> int | None:
    ...
    try:
        ...
        # PostgreSQL d'abord (étape 5) : cette pastille se rafraîchit à chaque ouverture
        # d'écran, c'est-à-dire souvent pendant qu'un scrape tient le verrou du pool.
        try:
            import pool_pg as _src
            etats = (_src.stats_for_site(site) or {}).get("by_state") or {}
        except Exception:
            import contacts_pool_backend as _duck
            etats = (_duck.stats_for_site(site) or {}).get("by_state") or {}
