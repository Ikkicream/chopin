...
def messagerie(site: str, seulement_non_lus: bool = False, limite: int = 100,
               seulement_lus: bool = False) -> dict:
    """Les messages vocaux, non lus d'abord.

    `seulement_lus` sert l'onglet Archives : une fois un message ecoute, il n'a plus rien
    a faire dans la liste de travail, mais il ne doit pas disparaitre pour autant. Les
    deux filtres sont independants et composables — les passer tous les deux ne rend
    rien, ce qui est la reponse juste a une demande contradictoire.
    """
    sql = """
        SELECT id, direction, statut, membre_nom, numero_onoff, numero_externe,
         ...
         WHERE site_code = %(site)s AND type = 'VM'
           AND (NOT %(f_non_lus)s OR lu_at IS NULL)
           AND (NOT %(f_lus)s OR lu_at IS NOT NULL)
         ORDER BY (lu_at IS NULL) DESC, debut DESC NULLS LAST
         LIMIT %(lim)s
    """
    lignes = _lignes(sql, {"site": site, "f_non_lus": bool(seulement_non_lus),
                           "f_lus": bool(seulement_lus),
