...
def journal_envois(site: str, cid: str) -> list[dict]:
    ...
    """
    # Journaux servis par PostgreSQL (fin du Lot 1). Le comptage y est fait par ENVOI
    # distinct et non par ligne : le 2026-08-22, une reprise de marquage avait écrit 316
    # lignes pour 160 envois réels, et cet écran aurait annoncé le double.
    try:
        import journal_pg
        return journal_pg.envois_par_jour(site, cid)
    except Exception as e:  # noqa: BLE001
        print(f"[campaign_engine] journal: PostgreSQL indisponible ({type(e).__name__}: {e}) "
              f"— repli DuckDB", flush=True)
    prefixe = f"{site}-{cid}-%"
    lignes: dict[str, dict] = {}
    c = _duck()          # repli : journaux d'envoi historiques
    try:
        try:
...
def _todays_allowance(camp: dict, today: _date) -> int:
    ...
    # Le plafond du jour est le PALIER DU JOUR, pas le maximum du plan. Sur une cadence
    # plate les deux se valent ; sur une montée en charge, prendre le maximum autorise
...
def _drop_recently_emailed(contacts: list[dict],
    ...
    # Deuxième barrière, servie par PostgreSQL : elle y gagne au passage de couvrir TOUS
    # les canaux. La version DuckDB n'interrogeait que `maildoso_sent`, donc un contact
    # servi par Sweego ou Emelia passait au travers de ce second rideau.
    try:
        import journal_pg
        blocked |= journal_pg.recemment_servis(emails, days)
        if not blocked:
            return contacts, 0
        kept = [ct for ct in contacts if (ct.get("email") or "").lower() not in blocked]
        return kept, len(contacts) - len(kept)
    except Exception as e:  # noqa: BLE001
        print(f"[campaign_engine] journal des envois illisible dans PostgreSQL "
              f"({type(e).__name__}: {e}) — repli DuckDB", flush=True)
    c = _duck()          # repli : journal maildoso historique
    try:
...
def _send_batch(camp: dict, contacts: list[dict], emails: list[str], today: _date) -> dict:
    ...
    if channel == "maildoso":
        ...
        # Qui a DÉJÀ été marqué. Sans cette trace, le filet de fin de lot re-marquait
        # tout le monde : `mark_pushed_to_emelia` écrit le journal PostgreSQL à chaque
        # appel, et le 2026-08-22 cela a produit 316 lignes d'envoi pour 160 emails
        # réellement partis — 160 posées au fil de l'eau, 156 reposées en quatre minutes
        # à la fin du lot. Aucun destinataire n'a reçu deux fois le message, mais tout
        # volume lu dans le journal comptait double, y compris celui qui pilote la montée
        # en charge des boîtes expéditrices.
        marques: set[str] = set()
        ...
        # callback — c'est-à-dire ceux qui portent le vrai risque, un email parti sans
        # cooldown ni ligne au journal, donc renvoyable demain.
        ok_emails = {(e or "").strip().lower() for e in res.get("sent_emails", [])}
        rattrapes = 0
        for ct in contacts:
        ...
        if rattrapes:
            print(f"[campaign_engine] {campaign_id} : {rattrapes} marquage(s) rattrapé(s) "
                  f"en fin de lot")
...
def reconcile_from_sent_log(cid: str, apply: bool = False) -> dict:
    ...
    site = camp["site_code"]
    emails, last_day = None, None
    try:
        import journal_pg
        emails, last_day = journal_pg.envois_de_campagne(site, cid)
    except Exception as e:  # noqa: BLE001
        print(f"[campaign_engine] rattrapage: PostgreSQL indisponible "
              f"({type(e).__name__}: {e}) — repli DuckDB", flush=True)
    if emails is None:
        c = _duck()          # repli : journal maildoso historique
        try:
