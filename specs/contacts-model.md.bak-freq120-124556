...
### 3.2 Cooldown cross-site (Q8.2)
Quand un site veut ajouter / pusher un contact :
```
last_contact_global = MAX(last_contacted_by_site_at) FROM contact_site_history WHERE contact_id = ?
if (today - last_contact_global) < COOLDOWN_DAYS:
    REFUSE l'ajout / le push
    + message : "contact contacté par {site_X} il y a {N} jours, attente cooldown"
```
**Valeur par défaut** : `COOLDOWN_DAYS = 30` (à valider).
...
WHERE
    ...
    AND (
        ...
        OR
        -- contacté il y a > 7 jours (Q3.2)
        csh.last_contacted_by_site_at < NOW() - INTERVAL 7 DAY
    )
    -- cooldown global
    AND NOT EXISTS (
...
1. **RGPD (B2B only, posture allégée)** — Genesis cible exclusivement le cold email B2B vers des contacts professionnels publics (sites web, annuaires, Pages Jaunes). La doctrine CNIL 2026 reste favorable pour le B2B sous réserve de : (a) opt-out fonctionnel dans chaque mail (géré par Emelia), (b) mention identité expéditeur dans le pied (cf. onboarding step 8), (c) sources publiques licites. Les garde-fous techniques retenus (blacklist globale, cooldown 30j) servent **autant la qualité business** (ne pas spammer un contact 3 fois en 1 mois) **que la conformité**. Pas de paranoïa "CNIL 4% CA" à entretenir.

2. **Concurrence cross-comptes** — Premier arrivé = locké 7 jours pour les autres (cooldown).
...
- [x] Cooldown global 30 jours, re-push même site 7 jours validés
