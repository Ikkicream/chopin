...
## 12ter. ✅ Confirmé en live (2026-09-22) — quota épuisé

- **`creation_date_min` / `creation_date_max` attendent une CHAÎNE**, pas un entier.
  `{"creation_date_min": 2025}` → `validation_error: "filters.creation_date_min" must be
  a string`. Écrire `"2025"`. C'est le filtre de la cible « sociétés récemment créées ».
- ⚠️ **`count` (`limit:1`) n'est PAS épargné par un 402.** « Gratuit en quota » signifie
  qu'il ne CONSOMME rien, pas qu'il passe quand le compte est bloqué : quota épuisé, même
  le comptage renvoie `quota_exhausted`. Conséquence pratique : dès que le mur est atteint,
  on ne peut plus rien dimensionner avant la remise à zéro — tout le travail de cadrage
  doit se faire AVANT d'épuiser le quota, pas après.
- **Le 402 porte le reliquat officiel** : `{"quota":{"monthlyLimit":250000,"remaining":0}}`.
  C'est la SEULE occasion où Basile dit où on en est — aucune réponse 200 ne le donne.
  `basile_backend._req` le saisit au vol et recale `basile_quota` dessus.
- **Rendement mesuré sur septembre 2026** : 6 973 contacts gardés pour 250 000
  enregistrements facturés = **36 facturés par contact utile**, soit 2,79 %. Conforme aux
  ~2 % annoncés plus haut : le forfait `api` plafonne donc la collecte à ~7 000
  contacts/mois, mur atteint vers le 20 du mois.
